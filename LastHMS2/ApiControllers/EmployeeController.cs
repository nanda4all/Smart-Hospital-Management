...
namespace LastHMS2.ApiControllers
{
    ...
    public class EmployeeController : ControllerBase
    {
        ...
        public EmployeeController(ApplicationDbContext context, IWebHostEnvironment hosting)
        {
            _context = context;
            _hosting = hosting;
        }
        ...
        public async Task<IActionResult> CreateSurgery([FromForm] IFormCollection fc, [FromQuery] DateTime date)
        {
            ...
            {
                status = true,
                message = "تم حجز غرفةالعمليات بنجاح"
            });
        ...
        public async Task<IActionResult> GetMedicalDetails([FromQuery] int id, [FromQuery] int PaId)//docId
        {
            ...
            var medical = await _context.Medical_Details.FirstOrDefaultAsync(m => m.Patient.Patient_Id == PaId);

            if (medical == null)
            {
                return Ok(new { status = false, Message = "لا يوجد ملف طبي", paId = PaId });
            }
            ...
            string PaName = _context.Patients.Find(PaId).Patient_Full_Name;
            return Ok(new
            {
                status = true,
                data = new
        ...
        {
            var test = await (from t in _context.Medical_Tests
                              join ty in _context.Test_Types
                              on t.Test_Type_Id equals ty.Test_Type_Id
                              where t.Medical_Detail_Id == medicalId
                              select new
                              {
                                  testId = t.Test_Id,
                                  testDate = t.Test_Date.ToString("dd-MM-yyyy"),
                                  testResult = t.Test_Result,
                                  testType = ty.Test_Type_Name,
                              }).ToListAsync();
            return Ok(test);
        }
        ...
        {
            var ray = await (from r in _context.Rays
        ...
        public async Task<IActionResult> CreateMedicalDetails([FromForm] IFormCollection fc, [FromQuery] List<string> allergies, [FromQuery] List<string> family, [FromQuery] List<string> chronic)
        {
            ...
            return Ok(new { message = "تم إضافة الملف الطبي بنجاح" });
